...
const CreateGenreForm: FC = () => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<GenreFormValues>({
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-6 border rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {createNewGenreMut.isPending ? <Loader2 className="animate-spin w-8 h-8" />: "Vytvoriť žáner"}
        </button>
      </form>
    </div>
  );
