...
const CreateGenreForm: FC = () => {
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-6 border rounded-lg shadow">
      <h1 className="text-3xl font-bold text-center mb-4">Vytvor nového spisovateľa/ku</h1>
      <Button variant={"link"}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {createAuthorMut.isPending ? <Loader2 className="animate-spin w-8 h-8" />: "Vytvoriť spisovateľa"}
        </button>
      </form>
    </div>
  );
