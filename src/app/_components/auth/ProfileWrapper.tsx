...
function BookGrid({ books, animate = true }: { books: any; animate?: boolean }) {
  return (
            ...
            <CardContent className="space-y-3 p-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className={getStatusColor(book.status)}>{new Date(book.dueDate).toLocaleDateString()}</span>
                </div>
...
const ProfileWrapper: FC = () => {
  ...
  const activeBooks = borrowedBooks.slice((activePage - 1) * ITEMS_PER_PAGE, activePage * ITEMS_PER_PAGE)

  const historyBooksPage = historyBooks.slice((historyPage - 1) * ITEMS_PER_PAGE, historyPage * ITEMS_PER_PAGE)
    ...
    <div className="min-h-screen bg-white dark:bg-background p-6">
      <div className="container mx-auto max-w-6xl">
        <motion.div
          ...
          className="mb-8 rounded-xl bg-white dark:bg-zinc-900 p-6 shadow-sm"
        >
        ...
        <Tabs defaultValue="borrowed" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 lg:w-[400px]">
            <TabsTrigger value="borrowed">Aktívne výpožičky ({borrowedBooks.length})</TabsTrigger>
            <TabsTrigger value="history">História výpožičiek ({historyBooks.length})</TabsTrigger>
          </TabsList>
